...
QImage DcmMonochromeImage::toQImage(const DcmImageTransferFunction &tf, int frame) const
{
    ...
    const DcmSignedByte *rawBytePtr = (const DcmSignedByte*) tagPixelDataPtr->constData();
    const DcmSignedShort *rawShortPtr = (DcmSignedShort*)rawBytePtr;

    int idx = frame * width() * height();

    // Somehow optimized, but still rather slow

    QMap<DcmSignedShort, QRgb> cache;

    if (bitsAllocated() > 8) {
        for (int y = 0; y < height(); y++) {
            QRgb *scanLine = (QRgb*)qImage.scanLine(y);
            for (int x = 0; x < width(); x++) {
                DcmSignedShort v = rawShortPtr[idx++];

                QRgb value;
                if (cache.contains(v)) {
                    value = cache.value(v);
                } else {
                    value = tf.colorForPixelValue(((double)v) * slope + offset).rgba();
                    cache.insert(v, value);
                }
        ...
        for (int y = 0; y < height(); y++) {
            QRgb *scanLine = (QRgb*)qImage.scanLine(y);
            for (int x = 0; x < width(); x++) {
                DcmSignedShort v = rawBytePtr[idx++];
